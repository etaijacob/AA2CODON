...
# Revisions copyright 2015 by Etai Jacob
# All rights reserved.
#
	...
	def parse(self):
		...
		def _parse_dbReference(element):
				...
				if 'type' in element.attrib:
					#e.g.
					#<dbReference type="RefSeq" id="NP_072179.1">
					#<property type="nucleotide sequence ID" value="NM_022650.2"/>
					#</dbReference>
					if element.attrib['type'] == 'RefSeq':
						nucid = '-'
						for ref_element in element:
							if ref_element.tag == NS + 'property':
								if ref_element.attrib['type'] == "nucleotide sequence ID":
									nucid = ref_element.attrib['value']
						self.ParsedSeqRecord.dbxrefs.append(element.attrib['type'] + ':' + element.attrib['id'] + ';' + nucid)
					#e.g.
					#<dbReference type="EMBL" id="KE346362">
					#<property type="protein sequence ID" value="EFW44694.2"/>
					#<property type="molecule type" value="Genomic_DNA"/>
					#</dbReference>
					elif element.attrib['type'] == 'EMBL':
						protid = '-'
						moltype = '-'
						for ref_element in element:
							if ref_element.tag == NS + 'property':
								if ref_element.attrib['type'] == "protein sequence ID":
									moltype = ref_element.attrib['value']
								if ref_element.attrib['type'] == "molecule type":
									protid = ref_element.attrib['value']
						self.ParsedSeqRecord.dbxrefs.append(element.attrib['type'] + ':' + element.attrib['id'] + ';' + moltype + ";" + protid)
					#e.g.
					#<dbReference type="EnsemblFungi" id="Mycgr3T40642">
					#<property type="protein sequence ID" value="Mycgr3P40642"/>
					#<property type="gene ID" value="Mycgr3G40642"/>
					#</dbReference>
					elif element.attrib['type'].startswith('Ensembl'):
						geneid = '-'
						protid = '-'
						for ref_element in element:
							if ref_element.tag == NS + 'property':
								if ref_element.attrib['type'] == "protein sequence ID":
									geneid = ref_element.attrib['value']
								if ref_element.attrib['type'] == "gene ID":
									protid = ref_element.attrib['value']
						self.ParsedSeqRecord.dbxrefs.append(element.attrib['type'] + ':' + element.attrib['id'] + ';' + geneid + ";" + protid)
					#e.g.
					...
					# </dbReference>
							...
							self.ParsedSeqRecord.dbxrefs.append(element.attrib['type'] + ':' + element.attrib['id'] + ';' + method + ';' + resolution + ';' + chains)
					else:
						self.ParsedSeqRecord.dbxrefs.append(element.attrib['type'] + ':' + element.attrib['id'])
				else:
					self.ParsedSeqRecord.dbxrefs.append(element.attrib['type'] + ':' + element.attrib['id'])
